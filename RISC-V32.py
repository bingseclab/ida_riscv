...
class riscv_processor_t(processor_t):
	id = 0x8922 #arbitrary number above 8000
	...
	## couldnt figure out a better way to handle the compressed instructions- there all pretty unique with a few exceptions
		...
		if op == 0:
			if funct3 == 000: #c.addi4spn
				...
				insn.Op2.addr = (BIT(w, 6) << 6) + (BITS(w, 12, 10) << 3) + (BIT(w, 5) << 2)	
			elif funct3 == 0b110: # c.sd
...
+ (BIT(w,  6) << 7) + (BITS(w, 5, 3) << 1) + (BIT(w,2) << 5) , 12)
				...
				insn.Op3.type = o_reg
				insn.Op3.reg = rs2
				if bits1110 == 0b00:  #c.srli
					insn.itype = self.itype_srli
					insn.Op3.type = o_imm
					insn.Op3.value = SIGNEXT((BIT(w, 12) << 5) + BITS(w, 6, 2), 6) 
					
				elif bits1110 == 0b10:  #c.andi
					insn.itype = self.itype_andi
					insn.Op3.type = o_imm
					insn.Op3.value = SIGNEXT((BIT(w, 12) << 5) + BITS(w, 6, 2), 6) 
				
				elif bits1110 == 0b01: #c.srai
					insn.itype = self.itype_srai
					insn.Op3.type = o_imm
					insn.Op3.value = SIGNEXT((BIT(w, 12) << 5) + BITS(w, 6, 2), 6) 

				elif bits65 == 0b00: insn.itype = self.itype_sub
				elif bits65 == 0b01: insn.itype = self.itype_xor
				elif bits65 == 0b10: insn.itype = self.itype_or
				elif bits65 == 0b11: insn.itype = self.itype_and
...
+ (BIT(w,  6) << 7) + (BITS(w, 5, 3) << 1) + (BIT(w,2) << 5) , 12)
		...
		#		insn.Op2.addr = (BIT(w, 12) << 5) + (BITS(w, 6, 5) << 3) + (BITS(w, 4,2) << 6)	
			...
			if (funct3 == 0b000):  #c.slli
				...
				insn.Op2.type = o_displ
				insn.Op2.reg = 2 #sp 
				insn.Op2.addr = (BIT(w, 12) << 5) + (BITS(w, 6, 4) << 2) + (BITS(w, 3,2) << 6)	
			...
			elif (funct4 == 0b1001) and (bits62 == 0) and (BITS(w, 11, 2) == 0):
				insn.itype = self.ebreak	
			...
			elif (funct4 == 0b1001) and (bits62 != 0): #c.add
			...
			elif funct3 == 0b110: # c.sdsp 
				insn.itype = self.itype_sw
				insn.Op1.type = o_displ
				insn.Op1.addr = (BITS(w, 12, 9) << 2) + (BITS(w, 8, 7) << 6)
